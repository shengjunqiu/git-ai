...
- 开发者 Git 信息和部门层级入口改为 dataset 参数。
- 部门可进入行使用 `.department-row-action` 表达可点击状态，不再使用 inline style。
- 所有模态框遮罩、关闭、取消、创建和保存按钮接入统一分发器。
- 遮罩只在 `event.target === actionElement` 时关闭，点击模态框内部不会误关闭。
- API Key 复制入口接入 action；创建成功后生成的按钮继续使用既有直接监听器。
- 30 个帮助命令复制按钮统一使用 `data-action="copy-help-command"`，保留剪贴板回退、
  临时“已复制”状态和 Toast。
- 删除已不再使用、专门为 JavaScript 字符串参数服务的 `jsString()`。

专项测试新增遮罩点击边界、开发者/部门 dataset 分发、帮助复制入口数量和源码零 inline
handler 断言。
...
| 指标 | 改造前 | 改造后 |
| --- | ---: | ---: |
| 静态 HTML inline handler | 61 | 0 |
| 动态模板 inline handler | 37 | 0 |
| 合计 | 98 | 0 |
