...
- [x] 把创建 `overviewTrendChart` 的配置提取到单独函数。
- [x] 没有实例时才创建 Chart。
- [x] 已有实例时更新 labels 和 datasets。
- [x] 自动刷新调用 `overviewTrendChart.update('none')`。
- [x] 首次加载和用户主动改变时间范围时可保留正常动画。
- [x] 数据从非空变为空时隐藏 canvas 并显示空状态。
- [x] 数据从空变为非空时恢复 canvas，必要时创建实例。
- [x] 仅在页面生命周期结束或确有必要时调用 `destroy()`。

验收标准：

- [x] 总览自动刷新时 Chart 实例引用保持不变。
- [x] 数据不变时图表没有动画或闪白。
- [x] 空数据和恢复有数据的状态切换正确。
...
- [x] 对 `trendChart` 应用相同的复用策略。
- [x] 当图表类型需要在单点 `bar` 和多点 `line` 之间切换时，采用类型变化时重建的保守策略。
- [x] 类型切换只在类型确实变化时重建，不在每次刷新时重建。
- [x] 自动刷新使用 `update('none')`。

验收标准：

- [x] 数据点数量和图表类型不变时不重建实例。
- [x] 单点与多点切换正确。
- [x] 指标和粒度选择器行为不变。
...
- [x] 复用 `agentComparisonChart`。
- [x] 更新 labels 和 dataset 后调用合适的 `update()`。
- [x] 自动刷新禁用动画。
- [x] 处理比较数据从有到无、从无到有的切换。

验收标准：

- [x] 趋势页面两个图表均不会在自动刷新时销毁重建。
- [x] 图例、坐标轴、颜色和横向柱状图布局保持不变。
...
- [x] 使用按栏目 `Map` 记录进行中的刷新。
- [x] 同一栏目已有自动刷新进行中时，跳过新的自动刷新。
- [x] 不使用会阻塞所有栏目的单一全局布尔锁。
- [x] 用户切换到新栏目时允许新栏目的首次加载立即开始。
- [x] 手动刷新与自动刷新冲突时，等待当前请求结束后只执行一次，并写入代码注释。
...
- [x] 慢请求超过 60 秒时不会叠加新的同栏目自动请求。
- [x] 快速切换栏目不会阻塞新栏目的首次加载。
- [x] 旧栏目的响应不会修改新栏目的“上次刷新时间”。

### 5.2 处理页面可见性

步骤：

- [x] 在 `document.hidden === true` 时跳过自动刷新。
- [x] 隐藏标签页时跳过请求，恢复可见后触发一次静默刷新。
- [x] `visibilitychange` 监听器只注册一次。
- [x] 恢复可见时仍遵守同栏目防重入规则。

验收标准：

- [x] 页面长期处于后台时不持续发送无意义请求。
- [x] 返回页面后数据会静默追平。
- [x] 恢复可见和定时器同时触发时，同栏目 AUTO 防重入会跳过第二轮。
...
> 2026-07-20 环境记录：当前没有运行中的 Enterprise 服务或已登录 Dashboard
> 会话。直接打开静态 `dashboard.html` 时，服务端模板占位符未渲染，且 `/static/*`
> 资源和 API 路由不存在，无法作为真实验收环境。以下矩阵保留未勾选，待启动完整服务并
> 准备管理员/非管理员测试账号后执行。
