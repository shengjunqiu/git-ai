...
- [ ] 每个错误保留 status、用户可读消息和可选 request ID。
- [ ] 不把服务端内部错误堆栈直接显示给用户。

验收标准：

- [ ] 调用者可以根据错误类型决定跳转、重试或保留旧数据。
- [ ] 错误日志有足够排查信息。
...
- [ ] 第一版可以先放在 `dashboard.js` 顶部，阶段 6 再移动到 `api.js`。
- [ ] 默认发送 `Accept: application/json`。
- [ ] 检查 HTTP status 和 Content-Type。
- [ ] 安全处理空 body、非 JSON body 和格式错误 JSON。
- [ ] 支持 `AbortSignal`。
- [ ] 支持显式超时。
- [ ] 401 统一跳转 `/auth/login?return_to=<current-url>`。
- [ ] 403 显示明确权限提示，不伪装成空数据。
- [ ] GET 仅在网络错误或明确可重试状态下有限退避重试。
- [ ] POST、PUT、DELETE 和文件上传默认不重试。
- [ ] 保留服务端返回的可读错误消息。

验收标准：

- [ ] 页面不再把 401 JSON 当作正常数据。
- [ ] 非 JSON 错误响应不会产生二次解析异常。
- [ ] 请求可以被栏目切换和超时取消。
...
- [ ] 搜索是否仍有意外裸 `fetch()`。
- [ ] 验证成功、401、403、500、网络断开和取消。

验收标准：

- [ ] Dashboard 业务代码不再直接调用裸 `fetch()`。
- [ ] 认证页若保留独立请求 helper，行为与 Dashboard 一致。

### 2.4 统一页面错误状态

步骤：

- [ ] 定义首次加载错误、后台刷新错误和 mutation 错误的不同展示。
- [ ] 首次加载失败显示栏目错误和重试按钮。
- [ ] 后台刷新失败保留旧数据并显示“数据可能已过期”。
- [ ] mutation 失败保留用户输入和操作上下文。
- [ ] 记录“最后尝试刷新”和“最后成功刷新”。
...
- [ ] 使用浏览器拦截分别返回 401、403、429、500、HTML 和空响应。
- [ ] 使用 Offline 模式验证网络错误。
- [ ] 使用 Slow 3G 验证超时和取消。
...
- [ ] 在页面头部增加移动端菜单按钮。
- [ ] 侧边栏改成可开关的抽屉，而不是 `display: none`。
- [ ] 增加遮罩层。
- [ ] 点击栏目、遮罩或 Escape 后关闭抽屉。
- [ ] 打开后把焦点移入菜单，关闭后恢复到触发按钮。
- [ ] 防止抽屉打开时背景滚动。
- [ ] 保留管理员栏目权限隐藏。
- [ ] 移动端提供可见的用户信息和退出入口。
...
- [ ] 为所有数据表增加统一 `.table-scroll` 容器。
- [ ] 避免 `.table-card { overflow: hidden }` 裁剪宽表格。
- [ ] 保持表头和单元格最小宽度。
- [ ] 评估首列 sticky，仅在不遮挡内容时启用。
- [ ] 在小屏上保证操作按钮不会被裁剪。
- [ ] 横向滚动容器具有可见焦点和可访问名称。

验收标准：

- [ ] 开发者、用户、API Key 等宽表格在手机上可完整访问。
- [ ] 页面本身不产生不可控的横向滚动。
...
- [ ] 390px 下统计卡从两列改为一列或自适应最小宽度。
- [ ] Toolbar 控件宽度允许换行并保持标签关联。
- [ ] 发布和文件表单按钮使用至少 44px 触控高度。
- [ ] 模态框适配软键盘和小屏高度。
- [ ] 帮助页代码块复制按钮不遮挡代码。
...
- [ ] 用户列表自动刷新不清除批量选择。
- [ ] 如果已选用户被删除或变为已授权，只移除失效选择。
- [ ] 部门层级保持不变。
- [ ] 分页页码和 cursor 保持不变。
- [ ] 自动刷新不关闭模态框、不清空表单、不改变焦点。
