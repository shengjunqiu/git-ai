...
| Qoder | Qoder CN 的 `QoderCN.exe`、`QoderCN` 进程和自定义安装目录未被检测，Hook 没有安装 | `src/mdm/agents/qoder.rs` 只检测 `Qoder.exe` 和 `qoder`，且只写国际版 `.qoder/settings.json`；CN 实际读取 `.qoder-cn/settings.json` | P0 |
...
7. 覆盖“应用已安装但对应的 `.qoder` / `.qoder-cn` 目录尚不存在”的首次安装场景。
...
- [x] 首次安装按产品成功创建 `%USERPROFILE%\.qoder\settings.json` 或 `%USERPROFILE%\.qoder-cn\settings.json`。
...
阶段状态：**Task 4.1 实现、单元测试和本机 D 盘 Qoder CN 检测已完成；Task 4.2 已由两次 GUI 测试依次修正 CN 配置根目录和 Windows Cmd runner，真实 Hook 已以 exit code 0 执行；Task 4.3 自动化归属闭环已完成，并根据第三次 GUI 测试修正 Qoder CN 模型存储目录与当前会话键。等待模型修正后的 GUI 归属验收，以及一套国际版 Qoder 环境。**
...
- Windows 卸载注册表检测按 `DisplayName` 精确区分 Qoder/Qoder CN，不依赖用户选择的安装盘符或目录；覆盖自定义安装、尚未启动且未加入 PATH 的首次安装场景。
- `process_names()` 增加 `QoderCN` 和 `Qoder CN`，Hook 更新后可识别需重启的 CN 进程。
- 安装器分别检测国际版 `.qoder/settings.json` 和 CN `.qoder-cn/settings.json`；首装创建对应产品目录，dry-run 不提前创建目录。
...
- 本机卸载注册表分别记录国际版 `Qoder` 和 `Qoder CN`，并提供各自实际 `InstallLocation`/`DisplayIcon`；生产检测只依赖稳定的产品显示名，不写死这些路径。
- 在 `.qoder` 完全不存在时，新 debug 二进制通过 `qoder-cn.cmd`/`QoderCN` 进程检测到产品，`install-hooks --dry-run` 报告 `Qoder: Pending updates`。
- 创建配置后再次 dry-run 报告 `Qoder: Hooks already up to date`。

### Task 4.2 执行记录：Windows Hook runner

执行环境最终由 Qoder CN 真实 Hook 日志确认：

- Qoder CN 内置 Go 后端虽包含 `buildCmdWindows`、`bashCommandCandidates` 和 `getInterpreter` 符号，但这些符号不足以证明实际 Hook runner 选择。
- 2026-07-16 17:24 的真实 `hooks.log` 明确记录 `actualCmd=cmd /c ...`；Windows Qoder Hook 因此改用 Cmd renderer，macOS/Linux 保持 POSIX renderer。
- [Qoder 官方 Hooks 文档](https://docs.qoder.com/extensions/hooks) 确认 IDE/JetBrains/CLI 共享 Hook 配置，且配置变更后需重启 Qoder，不支持热加载。
- 最终 installer 命令已由真实 `cmd.exe /D /S /C` 执行；包含空格、`&`、`%`、单引号和 D 盘路径的情况均能正确传递 argv、stdin 和 cwd。
...
- 第一次迁移误写 `%USERPROFILE%\.qoder\settings.json`，Qoder CN 重启后 `hooks.log` 仍为 0 字节，实机日志与自动生成配置证明 CN 的用户根目录是 `%USERPROFILE%\.qoder-cn`。
- 最终 `%USERPROFILE%\.qoder-cn\settings.json` 保留原有 `enabledPlugins`，并为 PreToolUse/PostToolUse 写入 `D:\linewell-code\git-ai\target\debug\git-ai.exe checkpoint qoder --hook-input stdin`。
...
| 命令 | 结果 |
| --- | --- |
| `cargo test qoder --lib -- --test-threads 1` | 通过：12 passed，包含国际版/CN 配置路径分离、模型用户目录、CN 原配置保留、首装/dry-run、迁移、幂等与精确卸载。 |
| `cargo test --test integration qoder:: -- --test-threads 1` | 通过：11 passed，包含当前/旧版模型键解析，以及创建、编辑、终端 checkpoint -> commit -> blame 闭环。 |
| `cargo test --test integration windows_hook_commands:: -- --test-threads 1` | 通过：7 passed，包含 Qoder 最终命令的真实 Cmd 执行。 |
| `cargo build --bin git-ai` | 通过。 |
...
- 2026-07-16 17:06 重启并触发 Qoder CN 后，新的 `hooks.log` 仍为 0 字节，证明 Hook 未被加载，而不是 checkpoint 或 blame 转换失败。
- Qoder CN 启动时自动生成 `%USERPROFILE%\.qoder-cn\settings.json`，安装包也包含 `.qoder-cn` 和 `QODER_HOME` 字符串；因此先前只写 `.qoder` 是明确的 CN 路径错误。
- 修正后的安装器对国际版和 CN 分别建立目标配置集合；检测到两者时会同时检查/迁移，任一目标缺失 Hook 都不会误报 up-to-date。
- 修正前新二进制 dry-run 报告 `Qoder: Pending updates`；合并 CN Hook 后报告 `Qoder: Hooks already up to date`。
- CN 原配置备份位于 `D:\linewell-code\git-ai\target\qoder-cn-settings-before-hooks.json`。
- 第二次 GUI 验收证明 Hook 已加载，但旧 `/d/...` 命令被展开为 `cmd /c /d/...`，每次 Pre/Post 都以 exit code 1 失败，stderr 为“系统找不到指定的路径”。
- 该次失败没有生成 Qoder `AiAgent` checkpoint；随后 Qoder 编辑器保存 Hook 生成 `KnownHuman` checkpoint，与用户看到的“AI 代码被识别为人工”完全一致。
- 命令已迁移为 Windows 盘符路径 `D:\...\git-ai.exe`；Cmd 迁移前配置备份位于 `target\.qoder-cn-settings-before-cmd-hooks.json` 和 `target\.qoder-settings-before-cmd-hooks.json`。
- 第三次 GUI 验收中，Pre/Post Hook 均以 exit code 0 完成，`hello.html` 已生成 Qoder `AiAgent` checkpoint，但模型为 `unknown`。
- 实机存储证明 Qoder CN 使用 `%APPDATA%\QoderCN\User`，当前会话键为 `chat.modelMapSession.<session_id>`；该键的 `dfmodel` 可由全局模型配置解析为 `DeepSeek-V4-Flash`。
- 模型回退现同时扫描 `Qoder\User` 与 `QoderCN\User`，优先支持当前 `chat.modelMapSession` 键，并保留旧 `chat.modelConfig.session` 键兼容。
...
- 本机现已同时安装 Qoder 国际版与 Qoder CN；国际版的 `.qoder/settings.json`、`%APPDATA%\Qoder\User` 和启动日志目录均已确认，等待一次国际版 GUI Agent 编辑以验收真实 Hook 与模型。

## 20. 跨产品安装路径与国际版/CN 审计：2026-07-16

阶段状态：**Trae、CodeBuddy、Qoder 的生产检测均不再依赖本机盘符；Trae 国际版/CN 使用独立 Hook 配置目标，CodeBuddy 两版按实机日志与官方文档继续共享 Hook 配置。自动化与 D 盘构建已通过，外部用户配置未在本次审计中写入。**

### 路径策略

- Hook 中的 `git-ai` 路径来自运行时 `current_exe()`，不会把开发机的 `D:\linewell-code` 写入其他用户配置。
- 用户目录通过系统 home/config API 推导，不写死 `C:\Users\admin`。
- Windows 应用检测组合使用已有配置目录、标准用户级/机器级目录、PATH、精确进程名和卸载注册表 `DisplayName`；注册表结果由共享缓存读取，不保存安装盘符。
- 完全便携、无注册表、未进 PATH、从未启动且未生成配置的程序无法被操作系统通用发现；首次启动后会由进程或配置目录识别。
- Trae 和 CodeBuddy 首装 dry-run 不再提前创建用户配置目录。

### Trae 国际版与 CN

- 实机国际版位于 `D:\Trae\Trae.exe`，日志明确记录 `userDataDir=%USERPROFILE%\.trae` 和 `%APPDATA%\Trae\User`。
- 实机 CN 位于 `D:\Trae CN\Trae CN.exe`，日志明确记录 `userDataDir=%USERPROFILE%\.trae-cn` 和 `%APPDATA%\Trae CN\User`。
- 安装器现在分别检测产品并只生成对应的 `.trae/hooks.json`、`.trae-cn/hooks.json`；同时安装时两者都参与状态、迁移和卸载检查。
- 进程与候选路径覆盖 `Trae.exe`、`Trae CN.exe`、`TraeCN` 等精确变体；模型回退原有实现已同时扫描 `%APPDATA%\Trae\User` 与 `%APPDATA%\Trae CN\User`。

### CodeBuddy 国际版与 CN

- 实机国际版位于 `D:\CodeBuddy\CodeBuddy.exe`，程序数据目录为 `.codebuddy`。
- 实机 CN 位于 `D:\CodeBuddy CN\CodeBuddy CN.exe`，编辑器程序数据目录为 `.codebuddycn`；但 CN 插件日志明确监听 `%USERPROFILE%\.codebuddy\settings.json`。
- [CodeBuddy CN IDE Hooks 文档](https://www.codebuddy.cn/docs/ide/Features/Hooks)与[国际版 CLI Hooks 文档](https://www.codebuddy.ai/docs/cli/hooks)均指定用户 Hook 为 `~/.codebuddy/settings.json`，因此两版继续共用一个 Hook 目标，不创建未经产品读取的 `.codebuddy-cn/settings.json`。
- 应用检测新增国际版/CN 标准目录、PATH、精确进程名和卸载注册表；模型继续优先读取 Hook 提供的 transcript/model，不依赖编辑器安装目录。

### 验证

| 命令 | 结果 |
| --- | --- |
| `cargo test mdm::agents::trae::tests --lib -- --test-threads 1` | 通过：11 passed，包含双配置目标、双产品候选/进程与 dry-run 无副作用。 |
| `cargo test mdm::agents::codebuddy::tests --lib -- --test-threads 1` | 通过：10 passed，包含国际版/CN 候选/进程、共享设置目标与 dry-run 无副作用。 |
| `cargo test test_windows_uninstall_display_names_parser --lib -- --test-threads 1` | 通过：1 passed。 |
| `cargo test --test integration trae:: -- --test-threads 1` | 通过：5 passed。 |
| `cargo test --test integration codebuddy:: -- --test-threads 1` | 通过：8 passed。 |
...
## 22. Windows commit 无 daemon 上传回退：2026-07-16

实机诊断确认，Windows `git commit` 已正常生成 `refs/notes/ai`，但当前配置为
`async_mode=false` 且后台 daemon 未运行。原同步路径中的 committed metrics 在找不到
daemon control socket 和进程内 telemetry worker 时被静默丢弃；CAS 虽进入本地队列，
也没有进程负责立即上传。因此重新执行 `install-hooks` 只是间接尝试启动 daemon，
并不是提交上传本身应有的依赖。

修正内容：

- telemetry 没有 daemon 路由时，先把 committed metrics 写入 metrics SQLite 队列，再进行一次无长时间重试的直接上传；网络或服务端失败时记录继续留在本地队列。
- post-commit CAS 在写入 `cas_sync_queue` 后，没有 daemon 时直接尝试上传；上传成功按 hash 清除队列，失败仍保留。
- CAS 上传前显式释放 internal DB 锁，避免成功响应后删除队列记录时发生同进程自锁。
- 无 daemon 的提交提示不再要求运行 `install-hooks`，改为说明已执行直接上传且失败会留在本地。
- 本地回退已经扫描完整 metrics 队列，提交提示阶段不再对同一次服务端故障重复请求。
- 新增同步 wrapper 集成测试，由 mock API 同时断言 committed metrics 包含本次 commit SHA，且 prompt CAS 已收到。
- Windows 开发流程新增 `task dev:binary`：构建产物仍在 D 盘，只刷新实际位于 `%USERPROFILE%\.git-ai\bin` 的 `git-ai.exe`/`git.exe` 代理，不重写编辑器 Hook，也不启动 daemon。
- Taskfile 的测试线程探测优先读取 Windows `NUMBER_OF_PROCESSORS`，其他平台命令均不可用时回退为 1，避免与测试无关的开发任务在 Windows 预解析变量时失败。

验证：

| 命令 | 结果 |
| --- | --- |
| `cargo test daemon::telemetry_worker::tests --lib -- --test-threads 1` | 通过：32 passed。 |
| `cargo test observability::tests --lib -- --test-threads 1` | 通过：10 passed。 |
| `cargo test --test daemon_mode synchronous_wrapper_post_commit_uploads_metrics_and_prompt_cas_without_daemon -- --test-threads 1 --nocapture` | 通过：1 passed。 |
| `cargo build --bin git-ai` | 通过，debug 产物位于 D 盘。 |

本机现有 daemon 模式基线测试仍在等待 Windows named pipe ready 阶段失败（broken pipe，
并观察到一次 daemon 进程 `0xc0000409` 退出）；该问题独立于本次同步回退。新测试明确
使用无 daemon wrapper 路径并已通过，修正目标正是让提交上传不再依赖该 daemon 状态。

### 实机提交与上传结果反馈

- 23:07 的测试提交 `ba24e058775f06ba66b4eb420f6c63b414d28515` 已正常生成 `authorship/3.0.0` Git Note，记录 CodeBuddy 的 11 行 AI 归属。
- 提交后 metrics WAL 已写入，但 5 条事件仍在待上传队列。刷新登录后手动重试时队列共有 6 条事件，服务端明确返回 `UploadForbidden: Developer is not authorized to upload Git tracking data`；CAS 的 1 条旧记录收到相同 403，并保留在本地等待授权。
- 因此 Windows 上传行为已经实际发生，未成功的直接原因是组织管理员未授权该开发者，而不是 Hook、登录状态或 daemon。
- 进一步修正同步反馈：`submit_local_telemetry` 不再丢弃本次 drain 结果，而是生成 typed `MetricsUploadResult`；plain wrapper 在同一线程保存该结果，由拥有用户输出的父 Git proxy 在 post-command hooks 结束后一次性取出并显示。
- 无 daemon 路径现在根据真实响应分别显示“上传成功”“管理员未授权”或“上传失败并保留 N 条”；同步模式复用实际请求结果，不额外探测服务端。daemon 异步模式继续保留原有授权探测。
- 代码归属上下文检索确认，原实现的目标是 best-effort 持久化，但 `let _ = drain_stored_metrics_queue(...)` 遗漏了同步结果反馈；相关上下文会话为 `0115b6c4cf9b845f`。
- 23:40 的第二次实机提交 `9b414b38196b2714f288ce90bfee5a23d109666f` 仍没有终端提示。自动化随后复现了完全相同的现象：mock API 已收到并接受 committed metrics，但 wrapper 的 captured stdout 中没有状态行。
- 第二层根因是输出所有权和 TTY 判断：深层 post-commit 不能可靠地向 Windows/IDE pipe 输出，且 `supress_output` 可能表示该执行者不拥有用户输出。最终由父 Git proxy 输出真实同步结果；已有真实结果不再受 TTY 判断限制，但仍尊重 `--quiet`、`--porcelain`、`--no-status` 和配置 `quiet`，避免脚本噪声及重复打印。

补充验证：

| 命令 | 结果 |
| --- | --- |
| `cargo test daemon::telemetry_worker::tests --lib -- --test-threads 1` | 通过：35 passed，新增真实成功、管理员拦截和网络失败提示测试。 |
| `cargo test observability::tests --lib -- --test-threads 1` | 通过：10 passed。 |
| `cargo test metrics::tests --lib -- --test-threads 1` | 通过：7 passed。 |
| `cargo test --test daemon_mode synchronous_wrapper_post_commit_uploads_metrics_and_prompt_cas_without_daemon -- --test-threads 1` | 通过：1 passed；除验证服务端收到 commit SHA/CAS 外，同时断言被捕获的 Git 输出包含真实上传成功状态。 |
| `cargo build --bin git-ai` | 通过。 |

## 23. Windows 手动更新实时安装输出：2026-07-17

Windows 不能在当前 `git-ai.exe` 仍运行时覆盖该可执行文件，因此安装阶段仍需由一个
能够在启动器退出后继续运行的 PowerShell 子进程完成。原实现对手动和自动更新都无条件
设置 `CREATE_NO_WINDOW`，并且只 `spawn()` 不等待；手动更新虽然没有主动把 stdout/stderr
设为 null，但隐藏进程使安装输出的稳定观测渠道只剩 `Start-Transcript` 日志。

修正后的行为：

- 显式 `git-ai update` 的安装器继续作为子进程完成自替换，但继承当前终端的 stdout/stderr，实时显示下载、文件锁等待、二进制替换、Hook/daemon 配置和最终成功或失败输出。
- 手动更新会明确提示：启动器即将退出释放文件锁，安装输出将继续出现在当前终端；用户应等待 `Successfully installed git-ai` 后再执行 Git 命令。
- daemon 自动更新继续使用 `CREATE_NO_WINDOW` 且将 stdout/stderr 指向 null，不弹窗、不污染宿主应用输出；完整 transcript 仍写入 `%USERPROFILE%\.git-ai\upgrade-logs\upgrade-<PID>.log`。
- 保留后台 helper 是 Windows 自替换的技术约束；隐藏手动安装输出不是约束，现已拆分。

验证：

| 命令 | 结果 |
| --- | --- |
| `cargo test commands::upgrade::tests --lib -- --test-threads 1` | 通过：54 passed，包含手动更新可见、自动更新隐藏的策略测试。 |
| `cargo build --bin git-ai` | 通过；仅有已记录的仓库基线 warning。 |
