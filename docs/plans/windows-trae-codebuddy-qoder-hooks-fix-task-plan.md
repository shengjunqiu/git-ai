...
- [ ] Windows 生成 PowerShell/native Windows 路径，配置中不再出现 `/c/.../git-ai.exe`。
- [ ] PowerShell 真实执行测试通过。
- [ ] 旧配置升级后只保留一个 PreToolUse 和一个 PostToolUse Git AI Hook。
- [ ] macOS/Linux 生成结果保持兼容。
...
- [ ] CodeBuddy CLI 的 Hook 仍能在 Git Bash 中执行。
- [ ] 两者使用同一配置时，重装不会互相覆盖成不可执行命令。
- [ ] 包含空格和特殊字符的 Windows 路径测试通过。
...
- [ ] CodeBuddy CLI 和 IDE 的写入、编辑、终端工具都有测试。
- [ ] IDE `PostToolUse` 生成 `CheckpointKind::AiAgent`。
- [ ] 只读工具仍为 Skip，人工编辑不会被扩大标记为 AI。
...
- [ ] `QoderCN.exe` 正在运行时 `tool_installed` 为 true。
- [ ] Qoder CN 位于自定义盘符且安装目录在 PATH 时 `tool_installed` 为 true。
- [ ] 未安装任何 Qoder 产品时不产生误报。
- [ ] 首次安装成功创建 `%USERPROFILE%\.qoder\settings.json`。
...
- [ ] 配置中的命令与实机 shell 一致。
- [ ] `create_file`、`search_replace` 和 `run_in_terminal` payload 均可进入 preset。
