...
fn validate_https_url(url: &str) -> Result<(), String> {
    ...
    Err(format!(
        "Security error: OAuth requires HTTPS. URL '{}' is not secure. Use https:// or http://localhost for local development.",
        url
    ))
}
...
mod tests {
    ...
    fn test_validate_https_url_http_rejected_in_release() {
        // In release mode, HTTP should be rejected for non-localhost
    ...
    #[cfg(not(debug_assertions))]
    #[test]
    fn test_validate_https_url_localhost_allowed_in_release() {
        // In release mode, HTTP should be allowed for localhost
        let result = validate_https_url("http://localhost:8080");
        assert!(result.is_ok());
    }
    ...
    #[cfg(not(debug_assertions))]
    #[test]
    fn test_validate_https_url_127_0_0_1_allowed_in_release() {
