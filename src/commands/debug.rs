...
fn build_debug_report() -> String {
    ...
    let _ = writeln!(out, "== Telemetry Upload ==");
    let diag = crate::daemon::telemetry_worker::telemetry_upload_diagnostics();
    let _ = writeln!(out, "API base URL: {}", diag.api_base_url);
    let _ = writeln!(out, "Using default API: {}", diag.using_default_api);
    let _ = writeln!(out, "Logged in: {}", diag.is_logged_in);
