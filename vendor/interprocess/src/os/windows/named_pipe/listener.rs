...
impl<Rm: PipeModeTag, Sm: PipeModeTag> PipeListener<Rm, Sm> {
    ...
    pub fn accept(&self) -> io::Result<PipeStream<Rm, Sm>> {
        ...
        // Wait after replenishing the stored instance and releasing the mutex.
        // This lets concurrent accept() callers each reserve an instance instead
        // of serializing every waiter behind the first connection.
        block_on_connect_clearing_empty_conns(instance_to_hand_out.as_handle())?;
